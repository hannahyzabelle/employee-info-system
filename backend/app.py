...
def trainings():
    ...
    conn.close()
    return render_template('trainings.html', trainings=data, search=search)
...
@app.route('/update_training/<int:id>', methods=['POST'])
...
def service():
    ...
    if search:
        data = conn.execute("""
            ...
            WHERE position LIKE ? OR agency LIKE ?
        """, ('%' + search + '%', '%' + search + '%')).fetchall()
    else:
        data = conn.execute("SELECT * FROM service_record").fetchall()
        
    conn.close()
    return render_template('service-record.html', records=data, search=search)
...
def update_service(id):
    ...
    conn.execute("""
        ...
        SET employee_id=?, position=?, start_date=?, end_date=?, agency=?
        WHERE id=?
    """, (
        request.form['employee_id'],
        request.form['position'],
